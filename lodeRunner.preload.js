...
function showLoadingPage() {
	...
	function addCover2Screen(image) {
		...
		addTitleBackground(
			(coverBitmap.getBounds().width * tileScale) | 0,
...
function preloadResource() {
	...
	var resource = [
		...
		{ src: "image/cursor/openhand.cur" + noCache, id: "openHand" }, //preload cursor
		{ src: "image/cursor/closedhand.cur" + noCache, id: "closeHand" },
	];
	...
	runnerSprite.setTransform(
		COVER_SIDE_X * tileScale,
	...
	function createMenuBitmapIcon() {
		...
		repeatActionIconObj = new repeatActionIconClass(
			screenX1,
			screenY1,
			tileScale,
			repeatActionOnIconBitmap,
			repeatActionOffIconBitmap
		);
...
var spriteSpeed = [
	...
	{ runnerSpeed: 0.7, guardSpeed: 0.35, digSpeed: 0.68, fillSpeed: 0.27, xMoveBase: 8, yMoveBase: 9 }, //ver 2
...
function createPreloadSpriteSheet() {
		...
		animations: {
			...
			fillHole: {
				frames: [
					...
					16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 17, 17, 18, 18, 19,
				], //delay fill time for champLevel, 2014/04/12
				next: false,
				speed: FILL_SPEED,
			},
		},
	...
	textData = new createjs.SpriteSheet({
		...
		animations: {
			...
			D2: 52,
			D3: 53,
			D4: 54, //blue digit number for player name, 11/17/2014
			D5: 55,
			D6: 56,
			D7: 57,
			D8: 58,
...
function createGuardObj(imageName) {
	var guard = new createjs.SpriteSheet({
		...
		animations: {
			...
			shakeRight: {
				frames: [8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 9, 10, 9, 10, 8],
				next: null,
				speed: GUARD_SPEED,
			},
