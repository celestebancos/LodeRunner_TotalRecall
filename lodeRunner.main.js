...
function enableStageClickEvent() {
	...
	stageClickListener = mainStage.on("click", function (evt) {
		stopDemoAndPlay();
	});
...
function disableStageClickEvent() {
	...
	if (stageClickListener) {
		rc = 1;
		mainStage.off("click", stageClickListener);
	}
...
function getLastPlayInfo() {
	...
	if (playMode == PLAY_URL) {
		...
		if (
			(playMode != PLAY_CLASSIC && playMode != PLAY_MODERN) ||
			playData < 1 ||
			(playData > maxPlayId && playData != PLAY_DATA_USERDEF)
		) {
...
var curScore = 0;
var curGetGold = 0,
	curGuardDeadNo = 0; //for modern mode
var sometimePlayInGodMode = 0; //if sometime play in god mode then don't save to hi-scoe, 12/23/2014

var infoY;
var scoreTxt, scoreTile, lifeTxt, lifeTile, levelTxt, levelTile, demoTxt;

var goldTxt, goldTile, guardTxt, guardTile, timeTxt, timeTile;
...
function drawScore(addScore) {
	...
	curScore += addScore;
	for (var i = 0; i < scoreTile.length; i++) mainStage.removeChild(scoreTile[i]);

	scoreTile = drawText(5 * tileWScale, infoY, ("000000" + curScore).slice(-7), mainStage);
}

function drawLife() {
	for (var i = 0; i < lifeTile.length; i++) mainStage.removeChild(lifeTile[i]);

	lifeTile = drawText(16 * tileWScale, infoY, ("00" + runnerLife).slice(-3), mainStage);
}

function drawLevel() {
	for (var i = 0; i < levelTile.length; i++) mainStage.removeChild(levelTile[i]);
...
function drawGold(addGold) {
	curGetGold += addGold;
	for (var i = 0; i < goldTile.length; i++) mainStage.removeChild(goldTile[i]);
...
function drawGuard(addGuard) {
	...
	for (var i = 0; i < guardTile.length; i++) mainStage.removeChild(guardTile[i]);
...
function drawTime(addTime) {
	countTime(addTime);
	for (var i = 0; i < timeTile.length; i++) mainStage.removeChild(timeTile[i]);
...
function startAllSpriteObj() {
	...
	if (curAiVersion < 3) {
		//for sprite only
		//(3) fill hole stop
		for (var i = 0; i < fillHoleObj.length; i++) fillHoleObj[i].play();
...
function stopAllSpriteObj() {
	...
	if (curAiVersion < 3) {
		//for sprite only
		//(3) fill hole stop
		for (var i = 0; i < fillHoleObj.length; i++) fillHoleObj[i].stop();
...
function gameOverAnimation() {
	...
	createjs.Tween.get(gameOverImage)
		...
		.wait(1500)
		.call(function () {
			if (gameState == GAME_OVER_ANIMATION) gameState = GAME_OVER;
		});
	/* 
		BUG: 
...
function incLevel(incValue, passed) {
	var wrap = 0;
	curLevel += incValue;
	while (curLevel > levelData.length) {
		curLevel -= levelData.length;
		wrap = 1;
	}
...
function mainTick(event) {
	...
	switch (gameState) {
		...
		case GAME_OVER:
			...
			showScoreTable(playData, scoreInfo, function () {
				showCoverPage();
			});
			gameState = GAME_WAITING;
			return;
