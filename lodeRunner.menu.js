...
function helpMenuClass(_stage, _bitmap, _editBitmap, _scale) {
	...
	function drawHelpMenu() {
		...
		helpB
			.beginFill("#190218")
			.drawRoundRect(startX + halfBorder, startY + halfBorder, bitmapW, bitmapH, halfBorder / 2)
			.endFill();
	...
	function drawCloseIcon(mouseOver) {
		...
		g.clear();
		//cross.alpha = 1;
		g.setStrokeStyle(closeBoxSize / 4)
			.beginStroke(crosColor)
			.moveTo(0, 0)
			.lineTo(closeBoxSize, closeBoxSize)
			.closePath();
	...
	function closeHelpMenu() {
		...
		if (callBackFun)
			setTimeout(function () {
				callBackFun(callBackArgs);
			}, 10); //add setTimeout just don't cause mouse event (click) cascade!
	}
}
...
function selectDialog(
	...
	function drawCoverBackground() {
		...
		coverBackgroundObj.graphics
			.beginFill("black")
			.drawRect(0, 0, _parentStage.canvas.width, _parentStage.canvas.height)
			.endFill();
	...
	function setBackground() {
		...
		background.graphics
			...
			.drawRoundRect(0, 0, CANVAS_SIZE_X, CANVAS_SIZE_Y, 8 * _scale)
			.endFill();
	...
	function createSliderBoundLine() {
		...
		boundLine1.graphics
			.setStrokeStyle(2)
			.beginStroke(SLIDE_BOUNDLINE_COLOR)
			.moveTo(BORDER1 + BORDER2, TOP_BORDER1 + TOP_BORDER2 + BORDER2 + 1)
			.lineTo(SLIDE_AREA_X + BORDER1 + BORDER2, TOP_BORDER1 + TOP_BORDER2 + BORDER2 + 1)
			.endStroke();
		boundLine1.alpha = 0;
		...
		boundLine2.graphics
			...
			.lineTo(SLIDE_AREA_X + BORDER1 + BORDER2, CANVAS_SIZE_Y - BORDER1 - BORDER2 - 1)
			.endStroke();
		boundLine2.alpha = 0;
	...
	function createBorder1() {
		...
		vBorder1.graphics
			.beginFill(BORDER1_COLOR)
			.drawRect(BORDER1 + BORDER2 - 1, 0, SLIDE_AREA_X + 2, TOP_BORDER1 + TOP_BORDER2)
			.endFill();

		vBorder2.graphics
			.beginFill(BORDER1_COLOR)
			.drawRect(BORDER1 + BORDER2 - 1, CANVAS_SIZE_Y - BORDER1, SLIDE_AREA_X + 2, BORDER1)
			.endFill();
	...
	function createBorder2() {
		...
		vBorder1.graphics
			.beginFill(BORDER2_COLOR)
			.drawRect(BORDER1, TOP_BORDER1 + TOP_BORDER2, CANVAS_SIZE_X - 2 * BORDER1, BORDER2 + 1)
			.endFill();
		...
		vBorder2.graphics
			...
			.drawRect(
				BORDER1,
				CANVAS_SIZE_Y - BORDER1 - BORDER2 - 1,
				CANVAS_SIZE_X - 2 * BORDER1 - BORDER2,
				BORDER2 + 1
			)
			.endFill();
		...
		hBorder1.graphics
			...
			.drawRect(
				BORDER1,
				TOP_BORDER1 + TOP_BORDER2 + BORDER2,
				BORDER2 + 2,
				CANVAS_SIZE_Y - BORDER1 - BORDER2 - TOP_BORDER1 - TOP_BORDER2
			)
			.endFill();
		...
		hBorder2.graphics
			.beginFill(BORDER2_COLOR)
			.drawRect(
				CANVAS_SIZE_X - BORDER1 - BORDER2 - 1,
				TOP_BORDER1 + TOP_BORDER2 + BORDER2,
				BORDER2 + 1,
				CANVAS_SIZE_Y - BORDER1 - BORDER2 - TOP_BORDER1 - TOP_BORDER2
			)
	...
	function createTabs(tabsOnly) {
		...
		for (var i = 0; i < maxPages; i++) {
			var tabsName =
				("00" + (i * SLIDE_PAGE_ITEMS + 1)).slice(-3) +
				" - " +
			...
			tabs[i].on("mouseover", function () {
				dialogStage.cursor = "pointer";
				dialogStage.update();
			});
			tabs[i].on("mouseout", function () {
				dialogStage.cursor = "default";
				dialogStage.update();
			});
		...
		tabsLine.graphics
			...
			.lineTo(SLIDE_AREA_X + BORDER1 + 2 * BORDER2, TOP_BORDER1 + TOP_BORDER2)
			.endStroke();
		...
		function tabsActive(id) {
			...
			tabBackground.graphics
				.clear()
				.setStrokeStyle(1)
				.beginStroke(TABS_ACTIVE_LINE_COLOR)
				.beginFill(TABS_ACTIVE_FILL_COLOR)
				.moveTo(0, textHeight)
				.lineTo(textHeight / 2, 0)
				.lineTo(textHeight / 2 + textWidth, 0)
				.lineTo(textHeight + textWidth, textHeight)
				.lineTo(0, textHeight)
				.endFill();
		...
		function tabsInActive(id) {
			...
			tabBackground.graphics
				.clear()
				.setStrokeStyle(1)
				.beginStroke(TABS_INACTIVE_LINE_COLOR)
	...
	function setSlidePage(pages) {
		...
		function init() {
			...
			levelLoop: for (var y = 0; y < selectItemY; y++) {
			...
			background.on("mouseover", function () {
				dialogStage.cursor = "url('image/cursor/openhand.cur'), auto";
			});
		...
		function createSelectLevel(x, y) {
			...
			var textColor = SELECT_TEXT_COLOR,
				level = startLevel + id;
			...
			selectText[id] = new createjs.Text(("00" + level).slice(-3), SELECT_TEXT_SIZE + "px Arial", textColor);
		...
		function delSelectLevel(level) {
			...
			if (level <= editLevels) {
				...
				if (level == startLevel) {
					//end pages and no more levels in this pages
					maxPages--;
					if (maxPages > 0) {
						tabsOnly = 0;
		...
		function buildSelectMap(level, id) {
			...
			border.graphics
				.beginFill(SELECT_MOUSE_OVER_COLOR)
				.drawRoundRect(
					-borderSize,
					-borderSize,
					SELECT_SIZE_X + 2 * borderSize,
					SELECT_SIZE_Y + 2 * borderSize,
					borderSize
				);
			border.alpha = 0;
			...
			switch (playMode) {
				case PLAY_EDIT:
					if (editLevelData == _levelData) {
						//del box only for "custom levels", NOT for "LOAD" button
		...
		function level2Bitmap(levelMap) {
			...
			for (var y = NO_OF_TILES_Y - 1; y >= 0; y--) {
				for (var x = NO_OF_TILES_X - 1; x >= 0; x--) {
					...
					curTile.setTransform(
						x * BASE_TILE_X * MAP_SCALE,
						y * BASE_TILE_Y * MAP_SCALE,
						MAP_SCALE,
						MAP_SCALE
					);
		...
		function setLevelInfo(selectObj, level) {
			if (modernScoreInfo[level - 1] >= 0) {
				...
				var scoreText = new createjs.Text(
					modernScoreInfo[level - 1],
					"bold " + SELECT_TEXT_SIZE + "px Helvetica",
					SELECT_SCORE_COLOR
				);
				var startX = ((SELECT_SIZE_X - _checkBitmap.getBounds().width * _scale) / 2) | 0;
				var startY = ((SELECT_SIZE_Y - _checkBitmap.getBounds().height * _scale) / 4) | 0;
		...
		function selectClick(evt) {
			debug(
				"selectClick(0) x=" +
					...
					evt.stageY +
					"(" +
					Math.abs(firstPressMoveY - evt.stageY) +
					")"
			);
		...
		function selectMouseOver() {
			...
			if (playMode == PLAY_EDIT && editLevelData == _levelData) {
				//del box only for "custom levels", NOT for "LOAD"
	...
	function createTitle() {
		var titleText = new createjs.Text(
			_titleName,
			"italic bold " + TITLE_TEXT_SIZE + "px Helvetica",
			TITLE_TEXT_COLOR
		);
...
function menuDialog(_titleName, _itemList, _stage, _scale, _closeIconEnable, _closeCallBack, _args) {
	...
	function init() {
		...
		createItemList();
		if (_closeIconEnable) {
			closeIconObj = new closeIconClass(
				startX + menuX,
				startY,
				_stage,
				_scale,
				CLOSE_ICON_ACTIVE_COLOR,
				closeBox,
				null
			);
		}
	...
	function createText() {
		...
		titleTextObj = new createjs.Text(_titleName, "bold " + TITLE_TEXT_SIZE + "px Helvetica", TITLE_TEXT_COLOR);
		titleWidth = titleTextObj.getBounds().width;
		titleHeight = titleTextObj.getBounds().height;

		maxItemTextWidth = 0;
		for (var i = 1; i < _itemList.length; i++) {
			itemText[i - 1] = new createjs.Text(
				_itemList[i].name,
				"bold " + ITEM_TEXT_SIZE + "px Helvetica",
				ITEM_TEXT_COLOR
			);
	...
	function creatBackground() {
		...
		background2Obj.graphics
			...
			.drawRoundRect(
				startX + BUTTON_BACKGROUND_BORDER_SIZE,
				startY + BUTTON_BACKGROUND_BORDER_SIZE,
				menuX - BUTTON_BACKGROUND_BORDER_SIZE * 2,
				menuY - BUTTON_BACKGROUND_BORDER_SIZE * 2,
				8 * _scale
			)
			.endFill();
		...
		for (var i = 0; i < itemText.length; i++) {
			...
			border.graphics
				...
				.drawRoundRect(
					-BUTTON_BORDER_SIZE,
					-BUTTON_BORDER_SIZE,
					maxItemTextWidth + BUTTON_BORDER_SIZE * 2,
					ITEM_AREA_Y + BUTTON_BORDER_SIZE * 2,
					BUTTON_ROUND_RADIUD
				)
				.endFill();
			...
			button.graphics
				...
				.drawRoundRect(0, 0, maxItemTextWidth, ITEM_AREA_Y, BUTTON_ROUND_RADIUD)
				.endFill();
...
function levelPassDialog(
	...
	var scoreNameObj, scoreTextObj, hiScoreNameObj, hiScoreTextObj;
	...
	function initContent() {
		...
		hiScoreTextObj = new createjs.Text(
			("00000" + _hiScore).slice(-6),
			ITEM_TEXT_SIZE + "px Helvetica",
			ITEM_TEXT_COLOR
		);
